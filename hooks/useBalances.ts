...
const earthlingContractAddresses: Map = {
  137: "0x8778B7FD7e2480C6F9Ad1075Bd848B7Ce1b9d90C",

} as const;

const roboroverContractAddresses: Map = {
  137: "0xe148cb1b6b2B040FEA525856A9E2F6E25115189A",

} as const;

const nfw3cContractAddresses: Map = {
  137: "0xB0CbF85907896918aaf9Aa27773637c9F8b5b0cc",
...
function useBalances(): {
  ...
  const earthlingContract = {
    address: earthlingContractAddresses[chain?.id || 137],
    abi: BalanceABI,
  };

  const roboroverContract = {
    address: roboroverContractAddresses[chain?.id || 137],
    abi: BalanceABI,
  };

  const nfw3cContract = {
    address: nfw3cContractAddresses[chain?.id || 137],
    abi: BalanceABI,
  };
